package sid.cycles.cycles.common.apertiumV1;
...
import sid.cycles.cycles.common.Context;
import sid.cycles.cycles.common.TranslatablePair;
...
public class GetData {	
	...
	public static Context getContext(String root, String lang1, String lang2) {		
		...
		ArrayList<TranslatablePair> pairs = SPARQLsearches.obtainTranslationsFromRootWord(root, lang1);
		...
		if(!pairs.isEmpty()) {
			...
			for (String t : trans) {
				...
				trans2 = SPARQLsearches.obtainTranslationsFromURI(t);
				for (String t2 : trans2) {
					...
					if (!t2.equals(pairs.get(0).getSourceLabel()) && !allTargets.contains(t2)) {
						...
						trans3 = SPARQLsearches.obtainTranslationsFromURI(t2);
	...
	public static void createLexiconFile(String lang){
		String outputFile = "data/lexicons/lexicon-" + lang + "_APv1.txt";
		try {
			PrintWriter writer = new PrintWriter(new File(outputFile), "UTF-8");
			ArrayList<String> lexicon = SPARQLsearches.obtainLexiconFromLanguage(lang);			
